...
const MultiSelectWithChips = ({ label, options, selected, onChange, placeholder = "Select..." }) => {
  ...
  return (
    <div className="filter-group relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-2 py-2 bg-[#0F172A] border border-white/10 rounded-xl shadow-2xl max-h-60 overflow-y-auto backdrop-blur-xl">
          {options.length === 0 ? (
            ...
            options.map(option => (
              <div 
                key={option} 
                className={`px-4 py-2 text-xs flex items-center justify-between hover:bg-white/5 transition-colors ${selected.includes(option) ? 'text-sky-400 font-bold' : 'text-white/70'}`}
